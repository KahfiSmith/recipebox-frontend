...
- Frontend currently expects `POST /auth/login` and `POST /auth/refresh` success responses in nested form under `data`, specifically `data.user` and `data.tokens.accessToken`.
...
- Frontend currently expects `GET /api/v1/dashboard` responses in nested form under `data`, with `summary.recipeCount`, `summary.upcomingMealPlanCount`, `summary.pendingShoppingItemCount`, plus array fields `recipes`, `mealPlans`, and `shoppingItems`.
